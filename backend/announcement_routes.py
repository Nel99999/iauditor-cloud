...
async def create_announcement(
    ...
    # Allow developer, master, and admin roles
    allowed_roles = ["developer", "master", "admin"]
    if user["role"].lower() not in allowed_roles:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only Admin, Master, and Developer can create announcements"
        )
...
async def update_announcement(
    ...
    if user["role"].lower() not in allowed_roles:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only Admin, Master, and Developer can update announcements"
        )
...
async def delete_announcement(
    ...
    if user["role"].lower() not in allowed_roles:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only Admin, Master, and Developer can delete announcements"
        )

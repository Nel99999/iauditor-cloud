...
interface SearchResult {
  id: string;
  type: 'task' | 'user' | 'group' | 'inspection' | 'checklist' | 'workflow';
  title: string;
  description?: string;
  metadata?: {
    status?: string;
...
const GlobalSearch: React.FC<GlobalSearchProps> = ({ isOpen, onClose }) => {
  ...
  const getResultIcon = (type: SearchResult['type']): JSX.Element => {
  ...
  const getResultTypeBadge = (type: SearchResult['type']): string => {
